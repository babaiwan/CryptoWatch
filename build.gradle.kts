    ...
    pluginConfiguration {
        ...
        changeNotes = """
            <ul>
                ...
                <li>1.0.6 - 缓解「已连接但推送 0」: REST 拉取会阻塞流服务的单线程调度器,
                    导致订阅下发被不断推迟、收到的行情全被丢弃; 现将阻塞的网络请求
                    移出调度线程, 订阅与广播不再被拖住.</li>
                <li>1.0.5 - 缓解频繁「重连中」: 该网络下合约入口握手本身约 80% 失败,
                    因此缩短握手超时并加快重试节奏, 同时移除成功率 0% 的备用域名
                    （避免每次失败都多等一个超时窗口）.</li>
                <li>1.0.4 - WebSocket 由现货切换为合约: 支持 TAKE 等仅在合约上市的山寨币实时行情,
                    并增加合约端点轮换以绕过被污染的域名; 现货不存在的交易对不再导致连接反复断开.</li>
